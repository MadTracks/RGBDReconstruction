...
namespace Microsoft.Samples.Kinect.KinectFusionExplorer
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged, IDisposable
    {
        ...
        private System.Timers.Timer sstimer;
        ...
        /// Calculated point cloud frame from image integration
        /// </summary>
        //private FusionPointCloudImageFrame deltapointCloudFrame;
        ...
        private void WindowLoaded(object sender, RoutedEventArgs e)
        {
            ...
            //this.label1.Content=this.errorCalculation(0.0799752,74,59,0.0638731);
        }
        ...
        private void OnKinectSensorChanged(object sender, KinectChangedEventArgs e)
        {
            ...
            if (this.sensor != e.NewSensor)
            {
                ...
                if (null != e.NewSensor && KinectStatus.Connected == e.NewSensor.Status)
                {
                    ...
                    this.StartColorStream(colorImageFormat);
                }
            }
        }
        ...
        private void ProcessDepthData(DepthData depthData)
        {
            try
            {
                if (null != this.volume && !this.savingMesh)
                {
                    ...
                    //FusionDepthProcessor.DepthFloatFrameToPointCloud(this.depthFloatFrame,this.pointCloudFrame);
                    //FusionDepthProcessor.AlignPointClouds(this.pointCloudFrame,this.deltapointCloudFrame,FusionDepthProcessor.DefaultAlignIterationCount,,);
                    ...
                    if (trackingSucceeded)
                    {
                        ...
                        this.status.Fill = Brushes.Green;
                        this.label1.Content = Properties.Resources.FrameLabel+this.framecounter.ToString();
                        ...
                        encoder2.Frames.Add(BitmapFrame.Create(this.depthFloatFrameBitmap));

                        string time = System.DateTime.Now.ToString("hh'-'mm'-'ss", CultureInfo.CurrentUICulture.DateTimeFormat);

                        string myPhotos = Environment.GetFolderPath(Environment.SpecialFolder.MyPictures);

                        string path = Path.Combine(myPhotos,"\\Kinect Photos\\", "KinectSnapshot-" + time + ".png");

                        string path2 = Path.Combine(myPhotos, "\\Kinect Photos\\", "KinectSnapshotDepth-" + time + ".png");
                        ...
                        try
                        {
                            ...
                            //this.statusBarText.Text = string.Format("{0} {1} {2}", Properties.Resources.ScreenshotWriteSuccess, path, path2);
                        }
                        catch (IOException e)
                        {
                            this.ShowStatusMessage(e.Message);
                            //this.statusBarText.Text = string.Format("{0} {1} {2}", Properties.Resources.ScreenshotWriteFailed, path, path2);
                        }
        ...
        private void AllocateFrames()
        {
            ...
            // Allocate delta point cloud frame
            /*if (null == this.deltapointCloudFrame || this.width != this.deltapointCloudFrame.Width || this.height != this.deltapointCloudFrame.Height)
            {
                this.deltapointCloudFrame = new FusionPointCloudImageFrame(this.width, this.height);
            }*/
        ...
        private void TakeScreenshotButtonClick(object sender, RoutedEventArgs e)
        {
            if (null == this.sensor)
            {
                return;
            ...
            // Update manual reset information to status bar
            //this.ShowStatusMessage(Properties.Resources.TakeScreenshot);
        }

        private void StartScreenshotSequenceButtonClick(object sender, RoutedEventArgs e)
        {
            SetTimer();
            this.StartScreenshotSequence.IsEnabled = false;
            this.StopScreenshotSequence.IsEnabled = true;
        }


        private void StopScreenshotSequenceButtonClick(object sender, RoutedEventArgs e)
        {
            sstimer.Stop();
            sstimer.Dispose();
            this.StopScreenshotSequence.IsEnabled = false;
            this.StartScreenshotSequence.IsEnabled = true;
        }

        private void SetTimer()
        {
            sstimer = new System.Timers.Timer(2000);
            sstimer.Elapsed += OnTimedEvent;
            sstimer.AutoReset = true;
            sstimer.Enabled = true;
        }

        private double errorCalculation(double ref_model, double ref_real, double real_val, double pred_val)
        {

            double ref_model1mm = ref_model / ref_real;
            double true_val = real_val * ref_model1mm;
            double error_rate = Math.Abs(true_val - pred_val) / true_val;
            return error_rate;
        ...
        {
            this.TakeScreenshotButtonClick(sender, new RoutedEventArgs());
        }
